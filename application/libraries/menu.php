...
class Menu {
    ...
    public function dynamicMenu() {
        ...
        $user_flag = $CI->session->userdata('user_flag');
        ...
        if ($user_type != 1) {// query for employee user role            
            ...
            if($user_flag != 4){
			$user_menu = mysql_query("SELECT * FROM tbl_menu WHERE grole_id IS NULL ORDER BY sort");
		}else{
			$user_menu = mysql_query("SELECT * FROM tbl_menu where grole_id = 4 ORDER BY sort");
		}
            
        }
        $user_language = mysql_query("SELECT active_language FROM tbl_gsettings ORDER BY id_gsettings");
    ...
    public function buildMenu($parent, $menu, $language) {
        ...
        if (isset($menu['parents'][$parent])) {
            $html .= "<ul>\n";
             // echo '<pre>';print_r($menu['parents']);echo '</pre>';die;
            foreach ($menu['parents'][$parent] as $itemId) {  
                ...
                if (isset($menu['parents'][$itemId])) { //if condition is true
                    ...
                    if (!empty($language)) {
                    $html .= "<li id='".$itemId."' class='" . $opened . "'>\n  <a href='" . base_url() . $menu['items'][$itemId]['link'] . "'> <i class='" . $menu['items'][$itemId]['icon'] . "'></i><span>" . $menu['items'][$itemId][$language] . "</span></a>\n";
                    }else{
                        $html .= "<li id='".$itemId."' class='" . $opened . "'>\n  <a href='" . base_url() . $menu['items'][$itemId]['link'] . "'> <i class='" . $menu['items'][$itemId]['icon'] . "'></i><span>" . $menu['items'][$itemId]['English'] . "</span></a>\n";
                    }
