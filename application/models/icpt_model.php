 ...
 * @author NaYeM
 */
class icpt_Model extends MY_Model {
    ...
    public function all_icpt_info($id = NULL) {
        $this->db->select('*');
        
        $this->db->from('hr_users');
		$this->db->where('hr_users.discipline_id', 12);
    ...
    public function pe_bulklist($id = NULL){
	  
	    $this->db->select('*');
        
        $this->db->from('pe_bulk');
		$this->db->where('discipline_id', 21);
