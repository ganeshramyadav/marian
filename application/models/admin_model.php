...
class Admin_Model extends MY_Model {
   ...
   public function get_expiry_credentials(){ 
        $this->db->select('hr_employee_document.*', FALSE);
        $this->db->select('hr_users.*', FALSE);
        $this->db->from('hr_employee_document');
        $this->db->join('hr_users', 'hr_users.uid = hr_employee_document.uid', 'left');
       // $this->db->where('tbl_employee.department_id', $department_id);
        $query_result = $this->db->get();
        $result = $query_result->result();
        return $result;
