...
class Settings extends Super_Controller {
    ...
    public function general_settings($val = NULL) {
        ...
        $data['subview'] = $this->load->view('super/settings/general_settings', $data, TRUE);
        $this->load->view('super/_layout_main', $data); //page load
    }
    ...
    public function save_ginfo($id = NULL) {
        ...
        if (!empty($_FILES['logo']['name'])) {
            ...
            $val == TRUE || redirect('super/settings/general_settings');
            $data['logo'] = $val['path'];
        ...
        redirect('super/settings/general_settings');
    }
    ...
    public function set_working_days() {
        ...
        $data['subview'] = $this->load->view('super/settings/set_working_days', $data, TRUE);
    ...
    public function save_working_days() {
        ...
        set_message($type, $message);
        redirect('super/settings/set_working_days');
    }
    ...
    public function holiday_list($flag = NULL, $id = NULL) {
        ...
        if (!empty($id)) {
            ...
            if (empty($data['holiday_list'])) {
                ...
                set_message($type, $message);
                redirect('super/settings/holiday_list');
            }
        ...
        $data['subview'] = $this->load->view('super/settings/holiday_list', $data, TRUE);
        $this->load->view('super/_layout_main', $data);
    }
    ...
    public function save_holiday($id = NULL) {
        ...
        redirect('super/settings/holiday_list'); //redirect page
    }
    ...
    public function leave_category($id = NULL) {
        ...
        if (!empty($id)) {
            ...
            if (empty($data['leave_category'])) {
                ...
                $message = "No Record Found";
                set_message($type, $message);
                redirect('super/settings/leave_category');
            }
        }
        // retrive all data from db
        $data['all_leave_category_info'] = $this->settings_model->get();

        $data['subview'] = $this->load->view('super/settings/leave_category', $data, TRUE);
        $this->load->view('super/_layout_main', $data);
    }
    ...
    public function save_leave_category($id = NULL) {
        ...
        redirect('super/settings/leave_category'); //redirect page
    }
    ...
    public function notification_settings() {
        ...
        $data['subview'] = $this->load->view('super/settings/notification_settings', $data, TRUE);
        $this->load->view('super/_layout_main', $data);
    }
    ...
    public function set_noticifation() {
        ...
        redirect('super/settings/notification_settings'); //redirect page
    }

    public function update_profile() {
        $data['title'] = "Change Password";
        $data['subview'] = $this->load->view('super/settings/update_profile', $data, TRUE);
        $this->load->view('super/_layout_main', $data);
    }

    public function profile_updated() {
        $employee_id = $this->session->userdata('employee_id');
        $data['f_name'] = $this->input->post('first_name');
        $data['l_name'] = $this->input->post('last_name');
        $data['user_name'] = $this->input->post('user_name');
		
	$this->settings_model->_table_name = "tbl_user"; //table name
        $this->settings_model->_order_by = "user_id";
        $val = $this->settings_model->get_by(array('user_name' => $data['user_name'],), TRUE);
		
	if(!empty($val)){
		$type = "error";
		$message = "Information Not Successfully Updated! Please Try again to new UserName.";
	}else{
        ...
        set_message($type, $message);
        redirect('super/settings/update_profile'); //redirect page
    }
    ...
    public function view_event($id = NULL) {
        ...
        $data['subview'] = $this->load->view('super/settings/view_personal_event', $data, TRUE);
        $this->load->view('super/_layout_main', $data);
    }
    ...
    public function save_event($id = NULL) {
        ...
        set_message($type, $message);
        if (!empty($id)) {
            redirect('super/settings/view_event');
        } else {
    ...
    public function language_settings() {
        ...
        $data['subview'] = $this->load->view('super/settings/language_settings', $data, TRUE);
        $this->load->view('super/_layout_main', $data);
    }
    ...
    public function add_language() {
        ...
        set_message($type, $message);
        redirect('super/settings/language_settings'); //redirect page
    }
    ...
    public function set_phrase($language) {
        ...
        $data['subview'] = $this->load->view('super/settings/set_phrase', $data, TRUE);
        $this->load->view('super/_layout_main', $data);
    }
    ...
    public function add_menu_language($language) {
        ...
        set_message($type, $message);
        redirect('super/settings/set_phrase/' . $language); //redirect page
    }
    ...
    public function set_form($id = NULL) {
        ...
        set_message($type, $message);
        redirect('super/settings/database_backup');
    }
